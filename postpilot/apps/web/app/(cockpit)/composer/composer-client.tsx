    ...
    <>
      ...
      <div className="composer-layout">
        <section className="panel composer-source"><header className="panel-header"><div><h2>Source content</h2><p>Your starting point for every variant.</p></div></header><div className="panel-body composer-form">
          ...
          <div className="composer-actions"><Button icon="save" onClick={createPost} disabled={demoMode || saving}>{saving ? "Creating…" : schedule ? "Schedule post" : "Save draft"}</Button><Button variant="ghost" onClick={() => router.push("/calendar")}>Cancel</Button></div>
        </div></section>
        <section><div className="variant-editor-list">{selected.length ? selected.map((platform) => { const variant = variants[platform]; const limit = PLATFORM_META[platform].limit; const finalLength = publishedLength(variant.caption, variant.hashtags); const accounts = channels.filter((channel) => channel.platform === platform && channel.status === "active"); const supported = ACTIVE_PLATFORMS.includes(platform); const account = accounts.find((item) => item.id === variant.accountId); return <article key={platform} className="variant-editor"><header className="variant-editor-head"><span className="platform-badge"><PlatformIcon platform={platform} size="sm" />{PLATFORM_META[platform].label}</span><span className={`char-count ${finalLength > limit ? "over" : ""}`}>{finalLength.toLocaleString()} / {limit.toLocaleString()} final</span></header><div className="variant-editor-body"><div className="variant-preview"><div className="variant-preview-head"><span className="variant-preview-avatar" style={{ "--platform-color": PLATFORM_META[platform].color } as React.CSSProperties}>{(account?.displayName || PLATFORM_META[platform].label).slice(0, 1).toUpperCase()}</span><span className="variant-preview-who"><strong>{account?.displayName || `Not connected · ${PLATFORM_META[platform].label}`}</strong><small>{schedule ? `Scheduled ${date} ${time} NPT` : "Draft preview"}</small></span></div>{media ? <div className="variant-preview-media">{media.kind === "video" ? <video src={localPreview || media.publicUrl} muted /> : <img src={localPreview || media.publicUrl} alt="" />}</div> : null}{variant.title ? <p className="variant-preview-title">{variant.title}</p> : null}<p className="variant-preview-caption">{variant.caption}</p>{variant.hashtags ? <p className="variant-preview-hashtags">{variant.hashtags}</p> : null}</div>{supported ? <label className="field"><span>Publishing account</span><select value={variant.accountId} onChange={(event) => updateVariant(platform, "accountId", event.target.value)} disabled={demoMode || !accounts.length}><option value="">{accounts.length ? "Choose an account…" : "No active account"}</option>{accounts.map((account) => <option value={account.id} key={account.id}>{account.displayName || account.externalId}</option>)}</select>{!accounts.length ? <span className="field-error"><Icon name="alert" size={14} />Connect {PLATFORM_META[platform].label} before scheduling or publishing.</span> : null}</label> : <p className="audit-note"><Icon name="external" size={14} />Copy preparation only · publish with the native scheduler.</p>}<label className="field"><span>Title <span className="field-hint">optional</span></span><input value={variant.title} onChange={(event) => updateVariant(platform, "title", event.target.value)} disabled={demoMode} placeholder="Platform-specific title" /></label><label className="field"><span>Caption</span><textarea value={variant.caption} onChange={(event) => updateVariant(platform, "caption", event.target.value)} disabled={demoMode} rows={platform === "x" ? 4 : 6} /></label><label className="field"><span>Hashtags</span><input value={variant.hashtags} onChange={(event) => updateVariant(platform, "hashtags", event.target.value)} disabled={demoMode} />{contentProblem(platform, variant.caption, variant.hashtags) ? <span className="field-error"><Icon name="alert" size={14} />{contentProblem(platform, variant.caption, variant.hashtags)}</span> : null}</label></div></article>; }) : <div className="state-panel"><span className="state-icon"><Icon name="compose" /></span><h2>Choose a platform</h2><p>Select at least one destination to edit its version.</p></div>}</div></section>
      </div>
    </>
  );
}
