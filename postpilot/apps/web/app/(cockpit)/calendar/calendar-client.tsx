    ...
    <>
      ...
      <section className="panel calendar-shell">
        ...
        {loading ? <LoadingPanel label="Reading the publishing schedule…" /> : error ? <div className="panel-body"><ErrorPanel message={error} retry={load} /></div> : <>
          ...
          <div className="calendar-grid">{cells.map((cell) => { const cellPlatforms = [...new Set(cell.items.flatMap((item) => item.platforms as Platform[]))]; return <div key={cell.key} className={`calendar-day ${cell.inMonth ? "" : "outside"} ${cell.date === kathmanduDate() ? "today" : ""}`}><div className="calendar-date-row"><span className="calendar-date">{cell.day}</span>{cellPlatforms.length ? <div className="calendar-day-platforms">{cellPlatforms.map((platform) => <span key={platform} className="platform-dot" style={{ "--platform-color": PLATFORM_META[platform].color } as React.CSSProperties} title={PLATFORM_META[platform].label} />)}</div> : null}</div><div className="calendar-items">{cell.items.slice(0, 3).map((item) => <div key={item.id} className={`calendar-item ${item.status}`} title={`${item.topic || "Untitled post"} · ${item.status}`}>{new Intl.DateTimeFormat("en-US", { timeZone: "Asia/Kathmandu", hour: "numeric", minute: "2-digit" }).format(new Date(item.scheduledAt))} · {item.topic || "Untitled"}</div>)}{cell.items.length > 3 ? <span className="calendar-item">+{cell.items.length - 3} more</span> : null}</div></div>; })}</div>
          <div className="calendar-agenda">{agenda.length ? agenda.map((cell) => <div className="agenda-day" key={cell.date}><div className="agenda-date">{new Intl.DateTimeFormat("en-US", { month: "short", timeZone: "UTC" }).format(new Date(`${cell.date}T12:00:00Z`))}<strong>{cell.day}</strong></div><div className="agenda-items">{cell.items.map((item) => <div className="agenda-item" key={item.id}><strong>{item.topic || "Untitled post"}</strong><small>{new Intl.DateTimeFormat("en-US", { timeZone: "Asia/Kathmandu", hour: "numeric", minute: "2-digit" }).format(new Date(item.scheduledAt))}</small><div className="agenda-item-platforms">{(item.platforms as Platform[]).map((platform) => <PlatformIcon key={platform} platform={platform} size="sm" />)}</div><StatusBadge status={item.status} /></div>)}</div></div>) : <EmptyState icon="calendar" title="No scheduled posts this month" description="Schedule a post in Composer to place it on the calendar." action={<LinkButton href="/composer" icon="plus">Create post</LinkButton>} />}</div>
        </>}
      </section>
    </>
  );
